...
const TRAP_LIFETIME_MSECS = 7000;
...
function Maze(canvasObjectID, cellSize) {
  ...
  this.createTreasure = function (gold) {
    var row = Math.round(
      this.nrows / 2 + randomInt(this.nrows / 4) - this.nrows / 8
    );
    var col = Math.round(
      this.ncols / 2 + randomInt(this.ncols / 4) - this.ncols / 8
    );
  ...
  this.createGoblin = function () {
    ...
    return new Goblin(
      this.randomGoblinLocation(),
      movementPreferences[randomInt(movementPreferences.length)],
      speed
    );
  };
  ...
  this.burrowing = function (currentPath) {
    ...
    if (currentPath.length > 0) {
      ...
      if (neighbours.length) {
        // did we find any unvisited ones?
        ...
        this.setCellOptions(
          neighbour.cellLocation,
          changeDirection(neighbour.direction, TURN_BACK)
        ); // tunnel goes both ways
  ...
  this.drawMaze = function () {
    ...
    if (this.bombExploding)
      this.bombExploding = this.expandingBlastRadius(this.bombLocation);
  };
  ...
  this.dropBomb = function (canvas, mouseClick) {
    if (this.bombEnabled && this.bombRecharged) {
      this.bombLocation = new Location(
        mouseClick.clientX - this.size.left,
        mouseClick.clientY - this.size.top
      );
      this.dischargeBomb();
      // recharge time starts from now
      this.rechargeTimer = setTimeout(function () {
        maze.rechargeBomb();
      }, this.rechargeBombMSecs());
    }
  };
  ...
  this.drawExplosionRing = function (
    location,
    radius,
    thickness,
    radiusLimit,
    ...
    if (radius <= radiusLimit) {
      // only draw it if hasn't reached the edges
  ...
  this.rechargeBombMSecs = function () {
    ...
    return Math.max(
      2000,
      Math.pow(this.detectionRange(), 2) * 0.4 -
        this.triumphantKillCount * 100 +
        this.retiredCount * 1000
    );
  };
  ...
  this.redraw = function () {
    this.drawMaze();
    this.redrawTimerID = setInterval(function () {
      clearInterval(this.redrawTimerID);
    }, 10);
  };
  ...
  this.moveGoblinDetector = function (canvasObject, mouseEvent) {
    ...
    if (mouseEvent)
      this.mouseLocation = new Location(
        mouseEvent.clientX - this.size.left,
        mouseEvent.clientY - this.size.top
      );
  };
    ...
    if (this.goblins.size == 0) {
      ...
      if (this.traps)
        this.traps.forEach(function (trap) {
          this.destroyTrap(trap.ID);
        }, this); // reset all traps
      ...
      this.chests.forEach(function (chest) {
        chest.gold += GOLD_IN_CHEST;
      }); // add gold to every existing chest
  ...
  this.grantPortal = function () {
    ...
    displayHints(
      "Portals can relocate goblins for easier disposal.<br>Middle mouse click creates a blue entrance or orange exit."
    );
    tadaaa.play();
  };
  ...
  this.setPortal = function (canvasObject, mouseClick) {
    if (this.portalEnabled) {
      var cursorLocation = new Location(
        mouseClick.clientX - this.size.left,
        mouseClick.clientY - this.size.top
      );
      var cursorCell = this.whichCell(cursorLocation);
      var cellOptions = this.getCellOptions(
        cursorCell,
        PORTAL_ENTRANCE | PORTAL_EXIT | GOBLIN_TRAP | TREASURE
      );

      if (cellOptions & (PORTAL_ENTRANCE | PORTAL_EXIT)) {
        // removing an old one?
        ...
        if (!this.portal || this.portal.cellLocationOut) {
          // starting a new one

          if (this.getCellOptions(cursorCell, GOBLIN_TRAP | TREASURE) == 0) {
            // but not on a chest or trap
            ...
            this.portal.timeout = setTimeout(function () {
              maze.removePortal(maze.portal.ID);
            }, PORTAL_LIFETIME_MSECS);
          }
        } else {
          // finishing a new one
  ...
  this.removePortal = function (portalID) {
    if (this.portal && this.portal.ID == portalID) {
      //  make sure timeout doesn't destroy a different one
  ...
  this.setGoblinTrap = function (canvasObject, mouseClick) {
    ...
    if (
      !(room.options & (GOBLIN_TRAP | TREASURE | PORTAL_ENTRANCE | PORTAL_EXIT))
    ) {
      ...
      goblinsEntrapped.forEach(function (ID) {
        this.killGoblin(ID);
      }, this);
  ...
  this.destroyTrap = function (ID) {
    ...
    if (trap) {
      // check existence, since it may have already been destroyed by some other method
  ...
  this.killGoblin = function (ID) {
    ...
    if (goblin) {
      // sometimes the goblin is already dead (bombed?)
      ...
      setTimeout(function () {
        maze.clearCellOptions(cellLocation, DEAD_GOBLIN);
      }, CORPSE_LIFETIME_MSECS);
  ...
  this.goblinController = function () {
    ...
    this.goblins.forEach(function (goblin) {
      maze.moveGoblin(goblin);
    });
    ...
    if (!previousLooterCount && this.looterCount > 0) {
      ...
      displayHints(
        "Get your gold back by killing the goblin<br>before it reaches the edge of the maze."
      );
    }
    ...
    if (this.detectionRange() > DETECT_RANGE_FOR_BOMB && !this.bombEnabled)
      this.grantBomb(); // the bomb

    if (this.detectionRange() > DETECT_RANGE_FOR_PORTAL && !this.portalEnabled)
      this.grantPortal(); // portals somewhere in round 3
  };
  ...
  this.grantMoreTraps = function () {
    ...
    displayHints(
      "You can now lay " +
        this.maxTrapsAtOnce +
        " traps at once. <br>Kill more goblins to enable more traps."
    );
    tadaaa.play();
  };
  ...
  this.maxTrapsAllowed = function () {
    return Math.min(
      Math.round(this.killCount / GOBLIN_KILLS_PER_TRAP) + 1,
      MAX_TRAPS
    );
  };
  ...
  this.grantBomb = function () {
    ...
    displayHints(
      "Double-click to drop a bomb and kill all goblins in range.<br>Kill looters to reduce recharge time."
    );
    tadaaa.play();
  };
  ...
  this.moveGoblin = function (goblin) {
    ...
    if (goblin.otherDimensionFlag) {
      // special effects and processing for entering portal
      ...
      var goblinImage = goblin.triumphantFlag
        ? goblinTriumphPic
        : goblin.animationFrame(goblinPics);
      var inTheRoom = goblin.location.at(
        this.whichLocation(goblin.targetCellLocation)
      );
      if (inTheRoom) {
        // goblin has reached their target cell - what next?
        ...
        if (room.options & TREASURE) {
          // the goblin is at a chest
        ...
        if (room.options & GOBLIN_TRAP) {
          goblin.showing = FRAMES_PER_SECOND * 2; // takes effect upon release
          var theTrap = this.traps.get(room.trapID);
          if (theTrap.goblinsEntrapped.has(goblin.ID)) {
            // start digging animation for the first one
            if (theTrap.firstGoblin === goblin.ID) {
              this.ctx.drawImage(
                goblin.animationFrame(goblinDigPics),
                goblin.location.x,
                goblin.location.y
              );
              // with occasional sound
              goblin.digSound();
            }
            goblinStopped = true;
          } else {
            if (theTrap.goblinsEntrapped.size < MAX_TRAPPED_GOBLINS) {
              ...
              goblin.stuckVoice();
              goblinStopped = true;
              // first goblin destroys the trap in time
              if (theTrap.goblinsEntrapped.size === 1) {
                theTrap.firstGoblin = goblin.ID;
                setTimeout(function () {
                  maze.destroyTrap(room.trapID);
                }, TRAP_LIFETIME_MSECS);
              }
            } else {
              // this goblin frees all the others
        ...
        if (
          goblin.triumphantFlag &&
          this.isEdgeOfMaze(goblin.targetCellLocation)
        ) {
          // goblin can retire
          goblin.retired();
        ...
        goblin.location = this.calculateIntermediateLocation(
          goblin.location,
          this.whichLocation(goblin.targetCellLocation),
          goblin.framesPerCell()
        );
      }

      if (this.bombExploding && !(room.options & GOBLIN_TRAP)) {
        // trapped goblins are safe from bomb (paradoxically)

        if (
          distanceBetweenCoords(goblin.centreOfGoblin(), this.bombLocation) <
          this.blastRadius
        )
          this.killGoblin(goblin.ID);
      }

      if (!goblinStopped) {
        if (this.mouseLocation) {
          // check for detection or partial detection

          var threshold =
            distanceBetweenCoords(goblin.centreOfGoblin(), this.mouseLocation) -
            this.detectionRange();
          if (threshold < 0) {
            this.ctx.drawImage(
              goblinImage,
            ...
            if (inTheRoom) goblin.whisper(); // and audible
          } else if (threshold < 10)
            goblin.showing = Math.round(goblin.framesPerCell() / 2); // partially visible
        }

        if (goblin.showing) {
          var showPic = goblinGhostlyPic;
          if (goblin.triumphantFlag && goblin.showing > FRAMES_PER_SECOND)
            showPic = goblinTriumphPic;
  ...
  this.stealSomeGold = function (goblin, chestID) {
    ...
    var stolenGold = Math.min(
      randomInt(GOLD_STEAL_AMOUNT) + GOLD_STEAL_AMOUNT,
      chest.gold
    ); // goblins steal - who'd think!
  ...
  this.isEdgeOfMaze = function (cellLocation) {
    return (
      cellLocation.row == 0 ||
      cellLocation.row == this.nrows - 1 ||
      cellLocation.col == 0 ||
      cellLocation.col == this.ncols - 1
    );
  };

  //---------------------------------------------------

  this.calculateIntermediateLocation = function (
    location,
    targetLocation,
    framesPerCell
  ) {
  ...
  this.whichCell = function (location) {
    return new CellLocation(
      Math.floor((location.y - 2) / (this.cellSize + 1)),
      Math.floor((location.x - 2) / (this.cellSize + 1))
    );
  };
  ...
  this.drawVerticalBorder = function (value, x, y) {
    drawLine(
      this.ctx,
      x,
      y - this.lineOffset(value) + 1,
      x,
      y + this.cellSize + this.lineOffset(value),
      this.chooseDrawColor(value),
      this.chooseDrawThickness(value)
    );
  };
  ...
  this.drawHorizontalBorder = function (value, x, y) {
    drawLine(
      this.ctx,
      x - this.lineOffset(value),
      y,
      x + this.cellSize + this.lineOffset(value),
      y,
      this.chooseDrawColor(value),
      this.chooseDrawThickness(value)
    );
...
function Goblin(cellLocation, movementPreferences, speed) {
  ...
  this.animationFrame = function (picArray) {
    var picnum =
      Math.floor(this.lifespan / (FRAMES_PER_SECOND / 8)) % picArray.length;
    return picArray[picnum];
  };

  this.retired = function () {
    retiredSound.play();
  };

  this.digSound = function () {
    if (this.lifespan % FRAMES_PER_SECOND === 0) digSound.play();
  };

  this.whisper = function () {
    if (randomInt(50)) goblinWhispering[randomInt(4)].play();
    // fifty times more likely to play the first sounds
    else if (!this.triumphantFlag)
      goblinWhispering[randomInt(goblinWhispering.length)].play(); // otherwise any sound
  };
  ...
  this.stuckVoice = function () {
    if (!this.triumphantFlag)
      goblinStuckMsg[randomInt(goblinStuckMsg.length)].play();
  };
  ...
  this.moveAccordingToAI = function (options) {
    ...
    if (direction & options) {
      this.targetCellLocation = maze.findNeighbour(
        this.targetCellLocation,
        direction
      );
...
function CellLocation(row, col) {
  ...
  this.at = function (otherCellLocation) {
    return (
      otherCellLocation.row == this.row && otherCellLocation.col == this.col
    );
  };
}
...
function changeDirection(currentDirection, newCompassBearing) {
  ...
  if (newCompassBearing == RANDOM)
    return allDirections[randomInt(allDirections.length)];

  for (var i = 0; i < allDirections.length; i++) {
    if (currentDirection & allDirections[i])
      return allDirections[(i + newCompassBearing) % allDirections.length];
  }
